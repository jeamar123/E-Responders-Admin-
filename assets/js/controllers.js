app
	...
	.controller('userInfoController', [		
		...
		function controller( $scope , $http , $timeout , getServerData ){
			...
			$scope.user_info = {};
			$scope.account = {};
			$scope.hide_tab = false;
			$scope.user_info.bday = '';
			$scope.loading_screen = false;
			$scope.loading_msg = false;
			$scope.loading_success = false;
			$scope.disable_tab = false;
			$scope.respond_msg = false;
			get_user_info();

			$scope.profile_file_upload = function (file_img){
				fdata.append('userfile',file_img[0]);	
				$('#btn_profile_upload').show();
			}
			$scope.save_account_setting = function(){
				
				if ($scope.account.new_pass) {
					$http.post(getServerData.url+'user/update_user_account',$scope.account)
						.success(function(response){
							console.log(response);
							$scope.respond_msg = true;
						})
				}else if($scope.account.new_pass != $scope.account.retype_new_pass){
			...
			$scope.save_prof_img = function(){
				$scope.loading_screen = true;
				$scope.loading_msg = true;
				$.ajax({
					type: "POST",
					url: getServerData.url+'user/do_upload',
					data:fdata,
    				dataType: 'json',
					contentType: false,
					cache: false,
			...
			function get_user_info(){
				$http.get(getServerData.url + 'user/getUserInfo')
					.success(function onSuccess(response){
						...
						for(x in response){
							$scope.$broadcast( 'get_user_dept_id', response[x].dept_id );
			...
			io.socket.on( "load-all-data" , 
							...
							for(x in data_arr){
								...
								$http.get(getServerData.url + 'maps/get_current_request')
									.success(function(req_data){
										console.log(req_data.msg)
										var alert_dept_id;
										if (req_data.status != "error") {
											for(val of req_data){
				...
				function alert_tone(){
					var alert_html = '<embed class="alert_tone" src="'+getServerData.url+'assets/sound/emergency_siren.mp3" autoplay="true" loop="true" width="2" height="0">'+
						             ...
						             '</noembed>';					
					$('#alert_tone_container').html(alert_html);
				}
		}
	])		
	//  kuhaon niya ang dept ID sa current user
	...
	.controller('departmentLocationsController', [
		"$scope",
		"$http",
		"$element",
		"getServerData",
		function controller( $scope , $http , $element , getServerData ) {
			$scope.showBtn = false;
			var map = new google.maps.Map(document.getElementById('mapForAllDeparmtments'), {
                zoom: 12,
                center: new google.maps.LatLng(8.475956978198, 124.65500358955),
			...
			$scope.get_category = function(category){
				get_locations(category)
			}
			...
			function get_locations(category){
				...
				$http.get(getServerData.url + 'user/getUserInfo')
					.success(function(user_data){
						if (user_data[0]['user_type'] == 'Administrator') {
							$http.get( getServerData.url + 'maps/all_department_location/' )
								.success(function(locations){
									console.log(user_data);
									for (x = 0; x < locations.length; x++) {
										if (locations[x]['dept_id'] == user_data[0]['dept_id']) {
											map = new google.maps.Map(document.getElementById('mapForAllDeparmtments'), {
											    zoom: 12,
											    center: new google.maps.LatLng(locations[x]['lat'], locations[x]['lng']),
											    mapTypeId: google.maps.MapTypeId.ROADMAP
											});
										}
									}
						            for (i = 0; i < locations.length; i++) {
						            	if (locations[i]['dept_id'] == user_data[0]['dept_id']) {
							                marker = new google.maps.Marker({
							                    position: new google.maps.LatLng(locations[i]['lat'], locations[i]['lng']),
							                    map: map,
							                    animation: google.maps.Animation.BOUNCE,
							                    icon: getServerData.url + '/assets/img/marker-icon/temple-2.png'
							                });
						            	}else if((locations[i]['category'] == 'Hospital') && (locations[i]['dept_id'] != user_data[0]['dept_id'])){
							                console.log('Hospital ni..')
							                marker = new google.maps.Marker({
							                    position: new google.maps.LatLng(locations[i]['lat'], locations[i]['lng']),
							                    map: map,
							                    icon: getServerData.url + '/assets/img/marker-icon/hospital-building.png'
							                });
						            	}else if((locations[i]['category'] == 'Police') && (locations[i]['dept_id'] != user_data[0]['dept_id'])){
							                console.log('Police ni...')
							                marker = new google.maps.Marker({
							                    position: new google.maps.LatLng(locations[i]['lat'], locations[i]['lng']),
							                    map: map,
							                    icon: getServerData.url + '/assets/img/marker-icon/police.png'
							                });
						            	}else if((locations[i]['category'] == 'Fire Station') && (locations[i]['dept_id'] != user_data[0]['dept_id'])){
							                console.log('Fire Station ni ....')
							                marker = new google.maps.Marker({
							                    position: new google.maps.LatLng(locations[i]['lat'], locations[i]['lng']),
							                    map: map,
							                    icon: getServerData.url + '/assets/img/marker-icon/fire.png'
							                });
						            	}

						                google.maps.event.addListener(marker, 'click', (function(marker, i) {
						                    return function() {
						                        infowindow.setContent(locations[i]['dept_name']);
						                        infowindow.open(map, marker);
						                    }
						                })(marker, i));                         
						            }						
								})
						}else if(user_data[0]['user_type'] == 'Superadmin'){
							$scope.showBtn = true;
							$http.get( getServerData.url + 'maps/get_all_department/' + category )
								.success(function(locations){
						            ...
						            for (i = 0; i < locations.length; i++) {
						            	...
						            	}else if((locations[i]['category'] == 'Hospital')){
							                marker = new google.maps.Marker({
							                    position: new google.maps.LatLng(locations[i]['lat'], locations[i]['lng']),							                    
							                    icon: getServerData.url + '/assets/img/marker-icon/hospital-building.png'
							                });
						            	}else if((locations[i]['category'] == 'Police')){
							                marker = new google.maps.Marker({
							                    position: new google.maps.LatLng(locations[i]['lat'], locations[i]['lng']),							                    
							                    icon: getServerData.url + '/assets/img/marker-icon/police.png'
							                });
						            	}else if((locations[i]['category'] == 'Fire Station')){
							                marker = new google.maps.Marker({
							                    position: new google.maps.LatLng(locations[i]['lat'], locations[i]['lng']),							                    
							                    icon: getServerData.url + '/assets/img/marker-icon/fire.png'
							                });
		...
		function controller( $scope , $http , getServerData ){
			...
			$scope.delete_user = function(user_id){
				if (confirm('Are you sure you want to delete this user?')) {
					$http.post(getServerData.url+'user/delete_user',user_id)
						.success(function(response){
							console.log(response);
							display_all_user();
						})					
			...
			function display_all_user(){
				$http.get(getServerData.url+'user/display_all_users')
					.success(function(data){
	...
	.controller('mobileAlertController',[
		...
		function controller( $scope , $http , $timeout , getServerData , io ){
			...
			$scope.$on( "alert-action" ,
				function on ( evt , data ) {
					console.log(data.alert_id+'mew');
					$http.post( getServerData.url + 'maps/update_alert_action', data )
						.success(function(response){
							console.log(response);
							$scope.$emit('alarm_off', {'alarm' : 'off'});
							$(".infowindow_"+data.alert_id).hide();
							io.socket.emit( "load-finish", { category : 'alert_action'} );
					...
					if (data.action) {
						$http.post( getServerData.url + 'maps/update_alert_action', data )
							.success(function(response){
								...
								$scope.$emit('alarm_off', {'alarm' : 'off'});
								io.socket.emit( "alert-response", response );
							})						
					};					
			})
				...
				$http.get(getServerData.url + 'maps/get_current_request')
					.success(function(data){
						if (data != 'empty') {							
							...
							console.log('No mobile alert requests');
						}
	...
	.controller('requestRedirectionController',[
		"$scope",
		"$http",
		"getServerData",
		"io",
		function controller( $scope , $http , getServerData , io ){
			$scope.department_list = {};
			get_department_list();
			$scope.$on( "redirect-request" ,
				function on (evt,data){
					$http.post(getServerData.url + 'department/redirect_request' , data )
						.success(function(response){
							console.log(response);
							io.socket.emit( "load-finish", { category : 'alert_action'} );
							io.socket.emit( "load-finish", { category : 'load_notif'} );
							$scope.$emit('alarm_off', {'alarm' : 'off'});
							location.reload();
						})
				})
			function get_department_list(){
				$http.get(getServerData.url + 'department/get_dept_list')
					.success(function(lists){
						$http.get(getServerData.url + 'department/getLatLng')
							.success(function(latLng){
								// console.log(latLng[0].lat)
								for(x in lists){
									// lists[x].kilometers = calcDistance(new google.maps.LatLng(latLng[0].lat,latLng[0].lng),new google.maps.LatLng(lists[x].lat,lists[x].lng));
									calcDistance(new google.maps.LatLng(latLng[0].lat,latLng[0].lng),new google.maps.LatLng(lists[x].lat,lists[x].lng))
									var start = new google.maps.LatLng(latLng[0].lat,latLng[0].lng);
									lists.sort(function(a,b){
									    return ((calcDistance(start,new google.maps.LatLng(a['lat'], a['lng']))) - 
									        (calcDistance(start,new google.maps.LatLng(b['lat'], b['lng']))));
									});							
								}
								$scope.department_list = lists;
								console.log(lists);
						})					
					})				
			}
			function calcDistance(p1, p2){
			     return (google.maps.geometry.spherical.computeDistanceBetween(p1, p2) / 1000).toFixed(5);
			}	
		}
	])
	// For Notification
	.controller('notifController',[
		...
		function controller( $scope , $http , getServerData , io ){
			...
			function getNotif(){
				$http.get( getServerData.url + 'department/getNotification' )
					.success(function(result){
						console.log(result);
						$scope.notifCount = result.length;
						$scope.notifContent = result;
					})				
			}
		}
	])
